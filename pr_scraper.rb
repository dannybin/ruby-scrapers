...
SEC_PR_URL = "https://www.sec.gov/News/Page/RSS/Page/1356125649507"
SEC_SPEECH_URL = "https://www.sec.gov/News/Page/RSS/Page/1356125649549"
...
sec_pr_page = Nokogiri::XML(open(SEC_PR_URL))
sec_speech_page = Nokogiri::XML(open(SEC_SPEECH_URL))
...
sec_pr_articles = sec_pr_page.css('item')
sec_speech_articles = sec_speech_page.css('item')

sources = [{:source=>"SEC",:article=>sec_pr_articles}, {:source=>"SEC",:article=>sec_speech_articles}]

r.connect(:host=>"162.242.238.193", :port=>28015).repl
r.db("jurispect").table("press_releases").run

def scraper(source, articles) 
  agencies = {"Treasury" =>497, "Federal Reserve"=>188, "SEC"=>466}
  begin
    for i in 0..articles.length do
      ...
      if date_diff < 86400
        r.db("jurispect").table("press_releases").insert([{"title"=> curr_title, "link"=> curr_link,
         "topics"=> curr_topics, "publication_date"=>Time.parse(curr_pubdate), "summary"=>curr_summary,
...
#Separate scraper for federal reserve press releases due to different xlm format
begin
  for i in 0..fr_pr_articles.length do
    curr_link = fr_pr_articles[i].css("link").text
    curr_title = fr_pr_articles[i].css("title").text
    curr_topics = fr_pr_articles[i].xpath("cb:news").xpath("cb:keyword").text
    curr_pubdate = fr_pr_articles[i].xpath("dc:date").text
    curr_summary = fr_pr_articles[i].css("description").text
...
begin
  for i in 0..fr_speech_articles.length do
    ...
    if date_diff < 86400    
      r.db("jurispect").table("press_releases").insert([{"title"=> curr_title, "link"=> curr_link,
       "author"=> curr_author, "publication_date"=>Time.parse(curr_pubdate), "summary"=>curr_summary, 
       "creation_time"=>Time.now, "source"=>"Federal Reserve", "agency"=>188}]).run
    end
  end
rescue
